...
bool SortKriterij(Tim *tim1, Tim *tim2)
{
...
Liga::Liga(Liga &&l): max_br_timova(l.max_br_timova)
{
	timovi=new Tim*[l.broj_timova];
	for(int i=0; i<l.broj_timova; i++)
		timovi[i]=new Tim(*(l.timovi[i]));
...
{
	if(broj_timova>max_br_timova-1) throw std::range_error("Liga popunjena");
	try {
...
{
	std::sort(timovi,timovi+broj_timova,SortKriterij);
	for(int i=0; i<broj_timova; i++)
